...
from app.services.database import DecisionRiskEvidenceRecord, SetupEpisodeRecord, get_session, init_db
...
class ShadowEntryValidationTests(unittest.TestCase):
    ...
    def test_continuous_outcome_uses_executable_ask_bid_costs_and_deduplicates(self) -> None:
        collector = EpisodeOutcomeCollector(start_worker=False, persist_observations=False)
        collector.register_episode(
            "episode-a",
            state="PREPARED",
            observed_at=datetime(2026, 7, 20, 10, 0, 0),
            context={"option_token": 580001, "underlying_token": 260105, "target_1": 110, "stop_loss": 95, "quantity": 15},
        )
        entry = self._event(0, 580001, bid=99, ask=100, ltp=100, bid_depth=100, ask_depth=100)
        target = self._event(30, 580001, bid=111, ask=112, ltp=113, bid_depth=80, ask_depth=80)
    ...
    def test_insufficient_quote_coverage_and_ltp_only_markers_are_not_profit(self) -> None:
    ...
    def test_outcome_persistence_retries_and_dead_letters(self) -> None:
        ...
        collector = EpisodeOutcomeCollector(repository=FailingRepository(), start_worker=False, persist_observations=True)
        collector.register_episode(
            "episode-dead",
            state="OBSERVE",
            observed_at=datetime(2026, 7, 20, 10, 0, 0),
    ...
    def test_background_evidence_queue_full_retry_recovery_and_deduplication(self) -> None:
        ...
        rejected = queue.enqueue_decision({"decision_type": "extra", "final_state": "REJECTED"})
        ...
        session = get_session()
        try:
            self.assertEqual(session.query(DecisionRiskEvidenceRecord).filter_by(decision_id="recover-event").count(), 1)
            episode = session.query(SetupEpisodeRecord).filter_by(episode_key=reservation["episode_key"]).one()
    ...
    def test_reservation_recovery_preserves_minimum_crash_safe_intent(self) -> None:
        ...
        reservation = service.reserve_order_intent(
            ...
            authorized_quantity=15,
            order_mode="live",
            risk_decision={"approved_risk_amount": 1000, "estimated_total_loss_at_stop": 90},
        ...
        self.assertEqual(recovered[0]["order_intent"]["tradingsymbol"], self._signal().tradingsymbol)
        self.assertEqual(recovered[0]["evidence_event_id"], "event-1")
        self.assertTrue(reservation["reservation_token"])

    def test_broker_acceptance_then_local_failure_keeps_episode_locked_for_reconciliation(self) -> None:
        signal = self._signal()
    ...
    def test_baseline_transition_and_continuous_policy_boundaries(self) -> None:
        ...
        opposed = TransitionPreparationShadowPolicy().evaluate(
            self._context(five_minute_structure="bearish", five_minute_strongly_opposed=True), []
        )
        ...
        continuous = ContinuousTransmissionShadowPolicy().evaluate(self._context(), events)
    ...
    def test_event_time_replay_is_deterministic_and_hides_future_candles(self) -> None:
        ...
        events = [
            ReplayEvent("TICK", 580001, base + timedelta(seconds=2), base + timedelta(seconds=2, milliseconds=5), 3, bid=101, ask=102, ltp=101, bid_depth=30, ask_depth=30, instrument_master_version="im-v1", contract_available=True),
            ReplayEvent("TICK", 260105, base + timedelta(seconds=1), base + timedelta(seconds=1, milliseconds=5), 2, ltp=58011, instrument_master_version="im-v1", contract_available=True),
            ReplayEvent("TICK", 580001, base, base + timedelta(milliseconds=5), 1, bid=100, ask=101, ltp=100, bid_depth=30, ask_depth=30, instrument_master_version="im-v1", contract_available=True),
        ]
        first = replay.run(context=self._context(), events=events, data_version="ticks-v1", tick_capable=True, persist=False)
        second = replay.run(context=self._context(), events=list(reversed(events)), data_version="ticks-v1", tick_capable=True, persist=False)
        self.assertEqual(first["run_id"], second["run_id"])
        self.assertEqual(first["event_order"], [1, 2, 3])

        candles = [
            ReplayEvent("CANDLE", 260105, base, base, 1, candle_state="building", provenance="websocket", candle_complete_at=base + timedelta(minutes=1), instrument_master_version="im-v1"),
            ReplayEvent("CANDLE", 260105, base, base + timedelta(minutes=1), 2, candle_state="completed", provenance="generated", candle_complete_at=base + timedelta(minutes=1), instrument_master_version="im-v1"),
            ReplayEvent("CANDLE", 260105, base + timedelta(minutes=1), base + timedelta(minutes=2), 3, candle_state="completed", provenance="backfilled", candle_complete_at=base + timedelta(minutes=2), instrument_master_version="im-v1"),
        ]
        visible = replay.visible_candle_events(candles, as_of=base + timedelta(minutes=1, seconds=1))
        self.assertEqual([item.sequence for item in visible], [1, 2])
        bar_only = replay.run(context=self._context(), events=candles, data_version="bars-v1", tick_capable=False, persist=False)
        self.assertIn("continuous_option_transmission_shadow_v1", bar_only["disabled_policies"])

    def test_replay_rejects_ambiguous_candles_and_unavailable_contract_ticks(self) -> None:
        replay = EventTimeReplayService()
        base = datetime(2026, 7, 20, 10, 0, 0)
        with self.assertRaisesRegex(ValueError, "completion timestamp"):
            replay.run(
                context=self._context(),
                events=[ReplayEvent("CANDLE", 1, base, base, 1, candle_state="completed", instrument_master_version="im-v1")],
                data_version="bad",
                tick_capable=False,
                persist=False,
            )
        with self.assertRaisesRegex(ValueError, "before the replay contract"):
            replay.run(
                context=self._context(),
                events=[ReplayEvent("TICK", 1, base, base, 1, instrument_master_version="im-v1", contract_available=False)],
                data_version="bad",
                tick_capable=True,
                persist=False,
            )
    ...
    def test_policy_timing_waterfall_and_missed_reason(self) -> None:
        base = datetime(2026, 7, 20, 10, 0, 0)
        observations = {
            stage: {"timestamp": base + timedelta(milliseconds=index * 100), "executable_ask": 100 + index}
        ...
        reason = MissedOpportunityClassifier().classify(
            {"outcome": {"executable_data_sufficient": True}, "gates": {"second_confirmation_delayed": True}}
        )
    ...
    def test_audited_equity_uses_executable_bid_and_tracks_distinct_risks(self) -> None:
        ...
        closed_loss = SimpleNamespace(status="closed", net_pnl=-250, pnl=-250, updated_at=base, created_at=base, approved_risk_amount=100)
        ...
        self.assertAlmostEqual(snapshot.unrealized_pnl_executable_bid, (expected_exit - 100) * 15, delta=0.01)
    ...
    def test_stop_exit_liquidity_estimates_are_named_and_do_not_change_active_proxy(self) -> None:
        samples = [
            {
        ...
        self.assertEqual(set(result["estimates"]), {CURRENT_BOOK_PROXY, HISTORICAL_ADVERSE_MOVE_ESTIMATE, STRESS_EXIT_ESTIMATE})
    ...
    def test_chronological_folds_purge_embargo_and_risk_simulation_whole_lots(self) -> None:
        ...
        folds = PolicyEvaluationService().chronological_folds(episodes, maximum_horizon_seconds=86400)
        self.assertTrue(folds["training"] and folds["validation"] and folds["out_of_sample"])
        self.assertGreaterEqual(
            folds["validation"][0]["decision_at"] - folds["training"][-1]["decision_at"],
            timedelta(days=2),
        )
        simulations = RiskTierCounterfactualSimulationService().simulate(episodes, starting_equity=100000)
        self.assertEqual(set(simulations), {"1_percent", "2_percent", "3_percent", "5_percent"})
    ...
    def test_final_pre_order_profile_keeps_append_only_evidence_off_critical_path(self) -> None:
        queue = EvidencePersistenceQueue(max_size=100, start_worker=False)
        service = PreOrderRiskService(risk_management_service=PassingRisk(), evidence_queue=queue)
        ...
        self.assertLessEqual(p95, 50.0, f"safe final pre-order smoke p95 was {p95:.3f} ms")
