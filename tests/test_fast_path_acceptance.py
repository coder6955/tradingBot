...
class FastPathAcceptanceTests(unittest.TestCase):
    ...
    def _context(self, *, tick=None, created_at=None):
        ...
        service.refresh(
            ...
            completed_candles={"1minute": [{"last_completed_at": now.isoformat()}], "5minute": [{"last_completed_at": now.isoformat()}]},
            candidates={
                "bullish": {
    ...
    def test_latency_telemetry_exposes_targets_without_inventing_live_samples(self):
        ...
        self.assertEqual(report["acceptance_targets_ms"]["cached_candidate_decision_p95"], 100.0)
        self.assertIsNone(report["metrics"]["websocket_callback_duration"]["p95_ms"])
        self.assertIsNone(report["metrics"]["cached_candidate_decision_duration"]["p95_ms"])

    def test_stale_context_rejects_without_synchronous_fallback(self):
        created_at = datetime.now() - timedelta(seconds=settings.fast_scan_context_max_age_seconds + 1)
    ...
    def test_scheduled_scan_enforces_explicit_rest_call_budget(self):
        ...
        service.config = {"side": "BUY", "order_mode": "paper", "limit": 1, "place_orders": False}
    ...
    def test_overlapping_fast_events_do_not_scan_or_place_orders(self):
        ...
        service = AutoTraderService(
            ...
            order_service_factory=lambda: calls.__setitem__("orders", calls["orders"] + 1),
            fast_scan_context_service=Context(),
        )
        service.running = True
        service.config = {"side": "BUY", "order_mode": "paper", "limit": 1, "place_orders": True}
        ...
        self.assertEqual(status["suppressed_reasons"], {"fast_rescan_already_running": 1})
        self.assertEqual(service.recent_decision_events()[0]["event_type"], "fast_rally_dispatch")
    ...
    def test_fast_event_suppression_is_observable_when_auto_trader_is_stopped(self):
        ...
        result = service.request_fast_rescan(
            {"direction": "bullish", "move_pct": 0.09, "timestamp": datetime.now().isoformat()}
        )
        ...
        self.assertEqual(service.recent_decision_events()[-1]["event_type"], "fast_rally_dispatch")
    ...
    def test_fast_validation_rejects_any_database_io_and_records_decision(self):
        ...
        while service.status()["fast_rally"]["validation_running"] and time.time() < deadline:
            time.sleep(0.01)
    ...
    def test_identical_rejections_are_one_row_until_gate_or_five_minute_candle_changes(self):
        ...
        def save(marker, reason="premium_trigger_not_confirmed"):
            return repo.save_rejection(
                ...
                reasons=[reason],
                contract=contract,
                factor_scores={"multi_timeframe": {"frames": [{"timeframe": "5minute", "last_completed_at": marker}]}},
            )
    ...
    def test_only_one_and_five_minute_frames_are_used_and_future_rows_do_not_change_decision(self):
        replay_at = datetime(2026, 7, 21, 11, 0)
        session = get_session()
        try:
            for timeframe, minutes in (("1minute", 1), ("5minute", 5), ("15minute", 15), ("30minute", 30), ("day", 1440)):
                for index in range(24):
                    ...
                    session.add(
                        Candle(
                            symbol="BANKNIFTY",
                            timeframe=timeframe,
                            timestamp=replay_at - timedelta(minutes=(24 - index) * minutes),
                            open_price=close - 2,
                            high_price=close + 4,
                            low_price=close - 4,
                            close_price=close,
        ...
        session = get_session()
        try:
            for timeframe, minutes in (("1minute", 1), ("5minute", 5), ("15minute", 15)):
                session.add(
                    Candle(
                        symbol="BANKNIFTY",
                        timeframe=timeframe,
        ...
        self.assertEqual(set(before["responsibilities"]), {"1minute", "5minute", "tick"})
        self.assertEqual({frame["timeframe"] for frame in before["frames"]}, {"1minute", "5minute"})
    ...
    def test_multi_timeframe_direction_uses_latest_session_only(self):
                ...
                for index in range(20):
                    ...
                    session.add(
                        Candle(
                            symbol="BANKNIFTY",
                            timeframe=timeframe,
                            timestamp=datetime(2026, 7, 20, 14, 0) + timedelta(minutes=index * minutes),
                            open_price=close + 2,
                            high_price=close + 4,
                            low_price=close - 4,
                            close_price=close,
                ...
                for index in range(6):
                    ...
                    session.add(
                        Candle(
                            symbol="BANKNIFTY",
                            timeframe=timeframe,
                            timestamp=datetime(2026, 7, 21, 9, 15) + timedelta(minutes=index * minutes),
                            open_price=close - 2,
                            high_price=close + 4,
                            low_price=close - 4,
                            close_price=close,
        ...
        result = MultiTimeframeContextService().evaluate(symbol="BANKNIFTY", trend="bullish", as_of=replay_at)

        self.assertTrue(result["passed"], result)
        self.assertEqual({frame["samples"] for frame in result["frames"]}, {6})
        self.assertEqual({frame["direction"] for frame in result["frames"]}, {"bullish"})
