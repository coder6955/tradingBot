...
from app.services.active_price_feed import (
    ActiveTradePriceFeed,
    KitePollingPriceFeed,
    PriceTick,
)
from app.services.market_data_coordinator import MarketDataCoordinator
from app.services.executable_price_service import (
    ExecutableExitPrice,
    ExecutablePriceService,
)
...
class TradeExitService:
    ...
    def __init__(
        ...
        *,
        ...
        self.executable_price_service = (
            executable_price_service or ExecutablePriceService()
        )
    ...
    def evaluate_once(self, limit: int = 100) -> dict[str, Any]:
        ...
        return {
            ...
            "price_feed": self.active_price_feed.status()
            if self.active_price_feed
            else {"websocket_enabled": False},
    ...
    def _evaluate_trade(self, provider: KiteProvider, trade: Any) -> dict[str, Any]:
        ...
        if str(trade.status).lower() == "reconciliation_mismatch":
            return {
                ...
                "tradingsymbol": trade.tradingsymbol,
                "closed": False,
                "reason": "broker_reconciliation_mismatch",
            }

        current_tick = self._current_option_tick(provider, trade)
        if current_tick is None:
            reason = (
                self.active_price_feed.last_reason
                if self.active_price_feed and self.active_price_feed.last_reason
        ...
        execution = self.executable_price_service.for_long_exit(
            current_tick, quantity=quantity
        )
        ...
        if current_price is None:
            return {
                ...
                or "executable_exit_price_unavailable",
                **self._price_metadata(current_tick, execution=execution),
            }

        protective_stop = self._pending_or_filled_protective_stop(
            provider, trade, current_tick, price=current_price
        )
        if protective_stop is not None:
            return protective_stop

        decision = self._outcome_for_price(
            provider, trade, current_price, target_supported=execution.target_supported
        )
        ...
        if outcome is None:
            return {
                ...
                **self._price_metadata(
                    current_tick, execution=execution, decision=decision
                ),
            }
        ...
        partial = self._maybe_partial_squareoff(
            provider, trade, current_tick, outcome, execution=execution
        )
        ...
        squareoff = self._squareoff(
            provider, trade, current_price, outcome=outcome, tick=current_tick
        )
        close_price = current_price
        if str(trade.mode).lower() == "paper":
            paper_payload = (
                squareoff.get("paper") if isinstance(squareoff, dict) else None
            )
            if (
                isinstance(paper_payload, dict)
                and paper_payload.get("exit_price") is not None
            ):
        ...
        updated = self.trade_repository.close_trade(
            ...
            **self._price_metadata(
                current_tick, execution=execution, decision=decision, include_in_db=True
            ),
        )
    ...
    def _pending_or_filled_protective_stop(
        self, provider: KiteProvider, trade: Any, tick: PriceTick, *, price: float
    ) -> dict[str, Any] | None:
        if str(trade.mode).lower() != "live":
            return None
        ...
        if not self._stop_loss_crossed(trade, price):
            return None
        return self._handle_protective_stop_exit(
            provider, trade, protective_order_id, tick, "stop_loss"
        )
    ...
    def _record_price_excursion(self, trade: Any, tick: PriceTick) -> None:
        try:
            ...
            logger.exception(
                "Failed to update MFE/MAE for trade_id=%s tradingsymbol=%s",
                getattr(trade, "id", None),
                getattr(trade, "tradingsymbol", None),
            )
    ...
    def _squareoff(
        ...
        if quantity <= 0:
            return {
                "status": "live_squareoff_skipped",
                "message": "filled quantity is zero",
            }
    ...
    def _squareoff_quantity(
        self, provider: KiteProvider, trade: Any, exit_price: float, quantity: int
    ) -> dict[str, Any]:
        if trade.mode == "paper":
            return {
                "status": "paper_partial_closed",
                "exit_price": exit_price,
                "quantity": quantity,
            }
        transaction_type = "SELL" if str(trade.side).upper() == "BUY" else "BUY"
    ...
    def _submit_live_exit(
        ...
        *,
        ...
        if protective_order_id and outcome == "stop_loss":
            protective_result = self._handle_protective_stop_exit(
                provider, trade, protective_order_id, tick, outcome
            )
            if protective_result is not None:
                return protective_result
        if protective_order_id:
            cancel_result = self._cancel_protective_order_before_exit(
                provider, trade, protective_order_id
            )
            if not cancel_result.get("cancelled"):
                ...
                failed = self.trade_repository.mark_exit_failed(
                    int(trade.id),
                    reason=reason,
                    broker_payload={"protective_cancel": cancel_result},
                )
                self._alert(
                    f"CRITICAL: {reason} for {trade.tradingsymbol} trade_id={trade.id}"
                )
                return {
                    "trade_id": failed.id,
        ...
        closing = self.trade_repository.try_mark_closing(
            ...
            **self._price_metadata(
                tick, execution=execution, decision=decision, include_in_db=True
            ),
        )
        if closing is None:
            return {
        ...
        try:
            ...
            squareoff = self._squareoff(
                provider, trade, float(execution.executable_price or tick.price)
            )
            if self.latency_metrics is not None:
                self.latency_metrics.record_between(
    ...
    def _handle_protective_stop_exit(
        self,
        provider: KiteProvider,
        trade: Any,
        ...
        if order_status in {"rejected", "cancelled", "canceled", "failed"}:
            ...
            self._alert(
                f"CRITICAL: {reason} for {trade.tradingsymbol} trade_id={trade.id} order_id={protective_order_id}"
            )
            return {
                "trade_id": failed.id,
                "tradingsymbol": failed.tradingsymbol,
    ...
    def _cancel_protective_order_before_exit(
        self, provider: KiteProvider, trade: Any, protective_order_id: str
    ) -> dict[str, Any]:
        try:
            history = provider.order_history(protective_order_id)
        except Exception as exc:
        ...
        if status in {"complete", "filled"}:
            return {
                "cancelled": False,
                "reason": "protective_order_already_filled",
                "latest": latest,
            }
    ...
    def _evaluate_closing_trade(
        self, provider: KiteProvider, trade: Any
    ) -> dict[str, Any]:
        if str(trade.mode).lower() != "live":
            return {
                "trade_id": trade.id,
        ...
        return self._confirm_live_exit(
            ...
            if getattr(trade, "exit_order_id", None)
            else None,
    ...
    def confirm_live_exit_for_trade(self, trade_id: int) -> dict[str, Any]:
        ...
        if str(trade.mode).lower() != "live" or str(trade.status).lower() != "closing":
            return {
                "trade_id": trade.id,
                "closed": False,
                "reason": "trade_not_waiting_for_live_exit_confirmation",
                "status": trade.status,
            }
        return self._evaluate_closing_trade(provider, trade)
        ...
        *,
        ...
        try:
            ...
            return {
                "trade_id": trade.id,
                "closed": False,
                "reason": "exit_order_history_unavailable",
                "message": str(exc),
            }
        latest = history[-1] if history else {}
        order_status = str(latest.get("status") or "").lower()
        ...
        if order_status in {"rejected", "cancelled", "canceled", "failed"}:
            ...
            self._alert(
                f"CRITICAL: {reason} for {trade.tradingsymbol} trade_id={trade.id} order_id={exit_order_id}"
            )
            return {
                "trade_id": failed.id,
                "tradingsymbol": failed.tradingsymbol,
        ...
        filled_quantity = self._int(latest.get("filled_quantity")) or self._int(
            latest.get("quantity")
        )
        if expected_quantity > 0 and (
            filled_quantity is None or filled_quantity < expected_quantity
        ):
            self._alert(
                f"CRITICAL: live exit fill quantity incomplete for {trade.tradingsymbol} trade_id={trade.id}"
            )
            return {
                "trade_id": trade.id,
                "tradingsymbol": trade.tradingsymbol,
        ...
        if avg_price is None or avg_price <= 0:
            self._alert(
                f"CRITICAL: live exit average price unavailable for {trade.tradingsymbol} trade_id={trade.id}"
            )
            return {
                "trade_id": trade.id,
                "tradingsymbol": trade.tradingsymbol,
        ...
        if not position_check["position_zero"]:
            self._alert(
                f"CRITICAL: broker position not flat after exit for {trade.tradingsymbol} trade_id={trade.id}"
            )
            return {
                "trade_id": trade.id,
                "tradingsymbol": trade.tradingsymbol,
        ...
        updated = self.trade_repository.close_trade(
            ...
            **(
                self._price_metadata(price_tick, include_in_db=True)
                if price_tick
                else {}
            ),
        )
        ...
        if self.latency_metrics is not None:
            self.latency_metrics.record_between(
                "exit_trigger_to_fill",
                getattr(trade, "exit_requested_at", None)
                or getattr(trade, "price_timestamp", None),
                detail={
                    "trade_id": updated.id,
                    "mode": "live",
                    "outcome": outcome,
    ...
    def _broker_position_zero(
        ...
        matches = [
            row
            for row in rows
            if str(row.get("tradingsymbol") or "").upper() == target
            and str(row.get("exchange") or exchange).upper() == exchange
        ]
        if not matches:
    ...
    def _outcome_for_price(
        ...
        *,
        ...
        if str(trade.side).upper() == "SELL" and target_supported:
            ...
            return {
                "first_triggered": triggered[0] if triggered else None,
                "triggered_rules": list(dict.fromkeys(triggered)),
            }
        if target_supported:
            if trade.target_3 is not None and price >= float(trade.target_3):
                triggered.append("target_3")
        ...
        return {
            "first_triggered": unique[0] if unique else None,
            "triggered_rules": unique,
        }
    ...
    def _maybe_partial_squareoff(
        ...
        *,
        ...
        if (
            outcome not in {"target_1", "partial_target_1"}
            or not settings.enable_partial_booking
        ):
            return None
        quantity = int(
            trade.remaining_quantity
        ...
        updated = self.trade_repository.record_partial_exit(
            int(trade.id), quantity=partial_qty, exit_price=price
        )
        return {
            "trade_id": updated.id,
            "tradingsymbol": updated.tradingsymbol,
    ...
    def _time_exit_outcome(self, trade: Any, price: float) -> str | None:
        if (
            settings.exit_open_trades_before_close_minutes > 0
            and self._near_market_close()
        ):
            return "time_exit"
        profile = self._exit_profile(trade)
        configured_minutes = int(
            profile.get("time_stop_minutes") or settings.option_time_stop_minutes
        )
        ...
        if created_at is None:
            return None
        created_ist = (
            created_at.replace(tzinfo=ZoneInfo("Asia/Kolkata"))
            if created_at.tzinfo is None
            else created_at.astimezone(ZoneInfo("Asia/Kolkata"))
        )
        ...
        if trend_runner:
            effective_minutes = max(
                configured_minutes,
                round(configured_minutes * settings.option_time_stop_trend_multiplier),
            )
    ...
    def _trailing_exit_outcome(self, trade: Any, price: float) -> str | None:
        ...
        high_since_entry = self._high_since_entry(
            str(trade.tradingsymbol), trade.created_at
        )
        high_since_entry = max(
            high_since_entry,
            float(getattr(trade, "highest_price_during_trade", 0.0) or 0.0),
        )
    ...
    def _can_partial_at_r(self, trade: Any, price: float) -> bool:
        if (
            not settings.enable_partial_booking
            or self._has_partial_exit(trade)
            or str(trade.side).upper() != "BUY"
        ):
            return False
        entry = float(trade.average_price or trade.entry_price or 0.0)
        stop = float(trade.stop_loss or 0.0)
    ...
    def _trade_lot_size(self, trade: Any) -> int:
        ...
        try:
            ...
            payload = {}
        factors = (
            payload.get("signal_factor_scores", {}) if isinstance(payload, dict) else {}
        )
        contract = factors.get("contract", {}) if isinstance(factors, dict) else {}
        try:
            lot_size = (
                int(contract.get("lot_size") or 0) if isinstance(contract, dict) else 0
            )
    ...
    def _premium_atr(self, tradingsymbol: str, created_at: datetime | None) -> float:
        ...
        try:
            ...
            if created_at is not None:
                query = query.filter(
                    Candle.timestamp >= created_at.replace(tzinfo=None)
                )
            rows = list(
                reversed(query.order_by(Candle.timestamp.desc()).limit(20).all())
            )
        finally:
            session.close()
        if len(rows) < 2:
    ...
    def _invalidation_exit_outcome(
        self, provider: KiteProvider, trade: Any, price: float
    ) -> str | None:
        if str(trade.side).upper() != "BUY":
            return None
        if settings.enable_underlying_invalidation_exit and self._underlying_failed(
            provider, trade
        ):
            return "underlying_invalidation"
        if settings.enable_premium_invalidation_exit and self._premium_failed(
            trade, price
        ):
        ...
        try:
            if self.market_data_coordinator is not None:
                quote = self.market_data_coordinator.quote(
                    ["NSE:NIFTY BANK"], provider=provider
                )
            else:
    ...
    def _underlying_structure_failed(
        self, trade: Any, price: float, candles: list[Candle]
    ) -> bool:
    ...
    def _premium_failed(self, trade: Any, price: float) -> bool:
        candles = self._recent_option_candles(
            str(trade.tradingsymbol), trade.created_at, 12
        )
        if len(candles) < 4:
            return False
        closes = [float(candle.close_price) for candle in candles]
    ...
    def _high_since_entry(
        self, tradingsymbol: str, created_at: datetime | None
    ) -> float:
        if created_at is None:
            return 0.0
    ...
    def _recent_option_candles(
        self, tradingsymbol: str, created_at: datetime | None, limit: int
    ) -> list[Candle]:
        session = get_session()
        try:
            query = session.query(Candle).filter(
                Candle.symbol == tradingsymbol, Candle.timeframe == "5minute"
            )
            if created_at is not None:
                query = query.filter(
                    Candle.timestamp >= created_at.replace(tzinfo=None)
    ...
    def _near_market_close(self) -> bool:
        try:
            hour, minute = (
                int(part) for part in settings.market_close_time.split(":", 1)
            )
        except ValueError:
            return False
        ...
        return (
            close_at - timedelta(minutes=settings.exit_open_trades_before_close_minutes)
            <= now
            <= close_at + timedelta(minutes=5)
        )

    def _current_option_tick(
        self, provider: KiteProvider, trade: Any
    ) -> PriceTick | None:
        exchange = str(trade.exchange or settings.option_exchange)
        tradingsymbol = str(trade.tradingsymbol)
    ...
    def _sync_active_trade_subscriptions(
        self, provider: KiteProvider, trades: list[Any]
    ) -> dict[str, Any]:
        ...
        for trade in trades:
            token = self._int(getattr(trade, "instrument_token", None))
            if token:
                validation = self._validate_trade_instrument_token(
                    provider, trade, token
                )
                if validation["valid"]:
                    tokens.add(token)
                else:
        ...
        subscribed = (
            ...
            if tokens
            else {"subscribed": [], "reason": "token_missing"}
        )
        return {
            "requested_tokens": sorted(tokens),
    ...
    def _unsubscribe_closed_option_token(self, trade: Any) -> None:
        ...
        if not token:
            return
        still_open = any(
            self._int(getattr(item, "instrument_token", None)) == token
            for item in self.trade_repository.open_trades()
        )
        if not still_open:
    ...
    def _validate_trade_instrument_token(
        self, provider: KiteProvider, trade: Any, token: int
    ) -> dict[str, Any]:
        ...
        if cached is None:
            try:
                if self.market_data_coordinator is not None:
                    instruments = self.market_data_coordinator.instruments(
                        str(trade.exchange or settings.option_exchange),
                        provider=provider,
                    )
                else:
                ...
                return {
                    ...
                    "token": token,
                    "tradingsymbol": str(trade.tradingsymbol),
                    "reason": "instrument_validation_unavailable",
                    "message": str(exc),
                }
            for item in instruments or []:
        ...
        valid = (
            expected_symbol == actual_symbol and expected_exchange == actual_exchange
        )
        return {
            "valid": valid,
            "token": token,
            ...
            "expiry": str(cached.get("expiry"))
            if cached.get("expiry") is not None
            else None,
    ...
    def _price_metadata(
        ...
        *,
        ...
        if age is None:
            age = max(
                0.0,
                (ist_now_naive() - tick.timestamp.replace(tzinfo=None)).total_seconds(),
            )
        payload = {
        ...
        if include_in_db:
            return {
                **payload,
                "exit_rule_first_triggered": decision.get("first_triggered")
                if decision
                else None,
                "exit_triggered_rules": decision.get("triggered_rules", [])
                if decision
                else [],
                ...
                "exit_executable_price": execution.executable_price
                if execution
                else None,
                ...
                "exit_quote_timestamp": execution.quote_timestamp
                if execution
                else tick.timestamp,
            }
        return {
            **payload,
            ...
            "first_triggered_rule": decision.get("first_triggered")
            if decision
            else None,
