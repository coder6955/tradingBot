...
class StopExitLiquidityResearchService:
    ...
    def estimate(
        ...
        *,
        ...
        samples = [item for item in historical_adverse_observations if self._usable(item)]
        historical = self._historical(quantity, samples)
        stress = self._stress(quantity, current, samples)
        recommended = current
        if historical["validation_status"] == "VALIDATED" and float(historical.get("estimated_slippage_per_unit") or 0.0) > float(recommended.get("estimated_slippage_per_unit") or 0.0):
            recommended = historical
        if stress["validation_status"] == "VALIDATED" and float(stress.get("estimated_slippage_per_unit") or 0.0) > float(recommended.get("estimated_slippage_per_unit") or 0.0):
            recommended = stress
        return {
    ...
    def _historical(self, quantity: int, samples: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        fill_times = sorted(float(item.get("exit_time_seconds") or 0.0) for item in samples)
        spreads = sorted(float(item.get("spread") or 0.0) for item in samples)
        spread_expansions = sorted(float(item.get("spread_expansion") or 0.0) for item in samples)
        depth_disappearance = sum(1 for item in samples if bool(item.get("bid_depth_disappeared")))
        ...
        return {
            ...
            "estimated_slippage_per_unit": self._percentile(slippages, 0.90) if slippages else None,
            "estimated_fillable_quantity": min(quantity, int(self._percentile(depths, 0.10))) if depths else 0,
            "estimated_exit_time_seconds": self._percentile(fill_times, 0.90) if fill_times else None,
            "adverse_spread_percentile": self._percentile(spreads, 0.90) if spreads else None,
            "spread_expansion_p90": self._percentile(spread_expansions, 0.90) if spread_expansions else None,
            "bid_depth_disappearance_rate_percent": round(depth_disappearance / len(samples) * 100.0, 4) if samples else None,
            "worst_observed_slippage": max(slippages) if slippages else None,
            "quantity_exceeds_observed_depth": bool(depths and quantity > int(self._percentile(depths, 0.10))),
            "partial_fill_assumed": bool(depths and quantity > int(self._percentile(depths, 0.10))),
            "validation_status": "VALIDATED" if validated else "INSUFFICIENT_SAMPLE",
        }

    def _stress(self, quantity: int, current: dict[str, Any], samples: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        stressed_slippage = max([current_slippage * 2.0, *observed_slippage], default=current_slippage * 2.0)
        stressed_depth = min(observed_depth) if observed_depth else max(0, int(current.get("observed_bid_depth") or 0) // 2)
        return {
            ...
            "stress_assumptions": ["worst_observed_slippage", "half_current_depth_when_history_missing"],
            "validation_status": "VALIDATED" if len(samples) >= 100 else "RESEARCH_ONLY",
        }

    def _usable(self, item: dict[str, Any]) -> bool:
        try:
            return float(item.get("executable_bid_slippage")) >= 0 and int(item.get("bid_depth")) >= 0
