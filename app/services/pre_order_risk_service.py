...
class PreOrderRiskService:
    ...
    def __init__(
        ...
        *,
        ...
        self.episode_reservation_service = (
            episode_reservation_service or EpisodeReservationService()
        )
    ...
    def evaluate_and_reserve(
        ...
        *,
        ...
        risk_request = (
            ...
            if isinstance(factors.get("risk_request"), dict)
            else {}
        )
        requested_tier = str(
            (metadata or {}).get("requested_risk_tier")
            or risk_request.get("requested_tier")
            or TIER_1_BASE
        )
        quality_details = (
            execution_quality.get("details", {})
        ...
        context = RiskDecisionContext(
            ...
            expected_entry=float(
                quality_details.get("ask") or signal.entry_price or 0.0
            ),
            ...
            entry_depth_quantity=self._optional_int(
                quality_details.get("ask_depth_quantity")
            ),
            stop_exit_depth_quantity=self._optional_int(
                quality_details.get("bid_depth_quantity")
            ),
            ...
            trades_taken_today=int(
                (risk_guard.get("summary") or {}).get("trades") or 0
            ),
            open_positions=int(
                (risk_guard.get("open_exposure") or {}).get("open_trades") or 0
            ),
            existing_premium_exposure=float(
                (risk_guard.get("open_exposure") or {}).get("premium_exposure") or 0.0
            ),
            ...
            correlated_banknifty_positions=int(
                (risk_guard.get("open_exposure") or {})
                .get("by_symbol", {})
                .get("BANKNIFTY", 0)
            ),
        ...
        self._record_latency(
            ...
            policy_started,
            {"mode": mode, "requested_tier": requested_tier},
        )
        rejection_reasons = list(active_decision.rejection_reasons)
        if not risk_guard.get("passed", False):
            rejection_reasons.extend(
                [
                    "ACCOUNT_RISK_GATES_FAILED",
                    *[str(reason) for reason in risk_guard.get("reasons", [])],
                ]
            )
        ...
        if live_requested:
            ...
            if (
                ...
                and not settings.enable_broker_emergency_sl
            ):
        ...
        lot_size = int(signal.lot_size or 0)
        approved_quantity = (
            (approved_quantity // lot_size) * lot_size if lot_size > 0 else 0
        )
        if (
            approved_quantity <= 0
            and "RISK_BUDGET_BELOW_MINIMUM_LOT" not in rejection_reasons
        ):
        ...
        evidence_context = {
            ...
            "estimated_total_loss_at_stop_for_approved_quantity": round(
                estimated_loss, 2
            ),
        ...
        evidence_payload = {
            ...
            "transition_timestamps": {
                "pre_order_evaluated_at": ist_now_naive().isoformat(sep=" ")
            },
        }
        if rejection_reasons:
            ...
            existing_episode = self.episode_reservation_service.status(
                identity["episode_key"]
            )
            if existing_episode and existing_episode.get("state") in {
                "RESERVED",
                "ORDER_PENDING",
                "OPEN",
                "CLOSED",
                ...
                rejection_reasons.insert(
                    0, f"DUPLICATE_EPISODE_{existing_episode.get('state')}"
                )
                evidence_payload["gate_results"] = {
                    "rejection_reasons": list(dict.fromkeys(rejection_reasons))
                }
        ...
        self._record_latency(
            "atomic_episode_reservation_duration", reservation_started, {"mode": mode}
        )
        if not reservation.get("acquired"):
            ...
            duplicate_payload = {
                **evidence_payload,
                ...
                "transition_timestamps": {
                    "reservation_rejected_at": ist_now_naive().isoformat(sep=" ")
                },
            }
        ...
        if not evidence.get("accepted", True):
            ...
            return {
                "passed": False,
                "rejection_reasons": [
                    str(evidence.get("reason") or "EVIDENCE_QUEUE_UNAVAILABLE")
                ],
                "risk_decision": active_decision.to_dict(),
                "shadow_risk_decisions": shadow_decisions,
                "approved_quantity": 0,
    ...
    def _persist_evidence(
        ...
        *,
        ...
        if self.evidence_queue is not None:
            return self.evidence_queue.enqueue_decision(
                payload, episode_key=episode_key, event_id=event_id
            )
        result = self.evidence_repository.record_decision(
    ...
    def _register_outcome_episode(
        ...
        *,
        ...
        factors = signal.factor_scores if isinstance(signal.factor_scores, dict) else {}
        contract = (
            factors.get("contract") if isinstance(factors.get("contract"), dict) else {}
        )
        try:
            self.outcome_collector.register_episode(
                episode_key,
                state=state,
                observed_at=self._metadata_time(metadata, "setup_generated_at")
                or self._metadata_time(metadata, "armed_at"),
                context={
                    "underlying_token": (metadata or {}).get("underlying_token")
                    or factors.get("underlying_token"),
                    "option_token": signal.instrument_token
    ...
    def _session_eligible(self) -> bool:
        ...
        start_hour, start_minute = (
            int(value) for value in settings.market_open_time.split(":", 1)
        )
        end_hour, end_minute = (
            int(value) for value in settings.market_close_time.split(":", 1)
        )
        start = now.replace(
    ...
    def _dte(self, expiry: str | None) -> int | None:
        ...
        try:
            return (
                datetime.fromisoformat(str(expiry)).date() - ist_now_naive().date()
            ).days
        except ValueError:
            return None
    ...
    def _record_latency(
        self, name: str, started: float, detail: dict[str, Any]
    ) -> None:
        if self.latency_metrics is None:
            return
        self.latency_metrics.record(
            name, (time.perf_counter() - started) * 1000.0, detail=detail
        )
