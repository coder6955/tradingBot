...
class TradeRepository:
        ...
        *,
        ...
        try:
            ...
            if isinstance(response, dict):
                ...
                family = (
                    signal.factor_scores.get("setup_family")
            ...
            paper_entry_price = (
                float(response.get("entry_price") or signal.entry_price or 0.0)
                if isinstance(response, dict)
                else float(signal.entry_price or 0.0)
            )
            initial_entry_price = (
                paper_entry_price
                if mode == "paper"
            ...
            factors = (
                signal.factor_scores if isinstance(signal.factor_scores, dict) else {}
            )
            risk_decision = (
                factors.get("risk_decision")
                if isinstance(factors.get("risk_decision"), dict)
                else {}
            )
            episode = (
                factors.get("episode")
                if isinstance(factors.get("episode"), dict)
                else {}
            )
            record = TradeRecord(
                ...
                highest_price_during_trade=initial_entry_price
                if initial_entry_price > 0
                else None,
                lowest_price_during_trade=initial_entry_price
                if initial_entry_price > 0
                else None,
                ...
                risk_policy_version=str(risk_decision.get("risk_policy_version"))
                if risk_decision.get("risk_policy_version")
                else None,
                approved_risk_tier=str(risk_decision.get("approved_tier"))
                if risk_decision.get("approved_tier")
                else None,
                approved_risk_percent=float(
                    risk_decision.get("approved_risk_percent") or 0.0
                ),
                approved_risk_amount=float(
                    risk_decision.get("approved_risk_amount") or 0.0
                ),
                estimated_loss_at_stop=float(
                    risk_decision.get("estimated_total_loss_at_stop") or 0.0
                ),
            )
            session.add(record)
            session.commit()
    ...
    def suspected_test_artifacts(self, limit: int = 100) -> dict[str, Any]:
        ...
        try:
            rows = (
                self._suspected_test_artifact_query(session)
    ...
    def quarantine_suspected_test_artifacts(
        ...
        return (
            ...
            .filter(
                or_(
                    ...
                    TradeRecord.order_response_json.like(
                        '%"signal_factor_scores": {}%'
                    ),
                )
            )
        )
    ...
    def update_mfe_mae(
        ...
        *,
        ...
        try:
            ...
            if not self._apply_mfe_mae(
                record, price=price, price_timestamp=price_timestamp
            ):
                return record
        ...
        *,
        ...
        try:
            ...
            if exit_order_response is not None:
                record.exit_order_response_json = json.dumps(
                    exit_order_response, default=str
                )
                record.exit_order_status = str(
                    exit_order_response.get("status") or "submitted"
                )
            record.updated_at = ist_now_naive()
        ...
        *,
        ...
        try:
            ...
            updated = (
                ...
                .filter(
                    ~TradeRecord.status.in_(
                        ["closing", "closed", "exit_failed", "reconciliation_mismatch"]
                    )
                )
                .filter(
                    func.coalesce(TradeRecord.exit_attempt_count, 0)
                    < settings.live_exit_max_retry_count
                )
                .update(
                    {
                        ...
                        TradeRecord.exit_triggered_rules_json: json.dumps(
                            exit_triggered_rules or []
                        ),
                        ...
                        TradeRecord.exit_attempt_count: func.coalesce(
                            TradeRecord.exit_attempt_count, 0
                        )
                        + 1,
    ...
    def update_protective_order(
        ...
        *,
        ...
        try:
            ...
            record.protective_order_response_json = json.dumps(
                broker_payload, default=str
            )
    ...
    def mark_exit_failed(
        ...
        *,
        ...
        try:
            ...
            record.exit_order_response_json = json.dumps(
                broker_payload or {"reason": reason}, default=str
            )
    ...
    def close_trade(
        ...
        *,
        ...
        try:
            ...
            record.price_age_seconds = (
                price_age_seconds
                if price_age_seconds is not None
                else record.price_age_seconds
            )
            record.exit_rule_first_triggered = (
                exit_rule_first_triggered or record.exit_rule_first_triggered
            )
            if exit_triggered_rules is not None:
                record.exit_triggered_rules_json = json.dumps(
                    exit_triggered_rules, default=str
                )
            record.exit_ltp = exit_ltp if exit_ltp is not None else record.exit_ltp
            record.exit_best_bid = (
                exit_best_bid if exit_best_bid is not None else record.exit_best_bid
            )
            record.exit_best_ask = (
                exit_best_ask if exit_best_ask is not None else record.exit_best_ask
            )
            record.exit_executable_price = (
                exit_executable_price
                if exit_executable_price is not None
                else record.exit_executable_price
            )
            record.exit_depth_coverage = (
                exit_depth_coverage
                if exit_depth_coverage is not None
                else record.exit_depth_coverage
            )
            record.exit_spread_pct = (
                exit_spread_pct
                if exit_spread_pct is not None
                else record.exit_spread_pct
            )
            record.exit_execution_source = (
                exit_execution_source or record.exit_execution_source
            )
            record.exit_quote_timestamp = (
                exit_quote_timestamp or record.exit_quote_timestamp
            )
            ...
            record.exit_confirmed_at = ist_now_naive()
            self._apply_mfe_mae(
                record, price=exit_price, price_timestamp=price_timestamp
            )
            if exit_order_response is not None:
                record.exit_order_response_json = json.dumps(
                    exit_order_response, default=str
            ...
            qty = (
                ...
                else (record.filled_quantity or record.placed_quantity)
            )
            if record.average_price is not None:
                pnl = self.pnl_service.calculate(
                    ...
                    include_slippage=not (
                        settings.enable_execution_realism
                        and str(record.mode).lower() == "paper"
                    ),
                    include_spread=not (
                        settings.enable_execution_realism
                        and str(record.mode).lower() == "paper"
            ...
            if record.episode_key:
                (
                    ...
                    .filter(
                        SetupEpisodeRecord.episode_key == record.episode_key,
                        SetupEpisodeRecord.state == "OPEN",
                    )
                    .update(
                        {
    ...
    def record_partial_exit(
        ...
        *,
        ...
        try:
            ...
            remaining = int(
                record.remaining_quantity
                or record.filled_quantity
                or record.placed_quantity
                or 0
            )
            ...
            self._apply_mfe_mae(
                record, price=exit_price, price_timestamp=ist_now_naive()
            )
            ...
            record.slippage_cost = (
                float(record.slippage_cost or 0.0) + pnl.slippage_cost
            )
    ...
    def _record_counterfactual_outcome(self, record: TradeRecord) -> None:
        ...
        try:
            ...
            factors = (
                payload.get("signal_factor_scores", {})
                if isinstance(payload, dict)
                else {}
            )
            shadow = (
                factors.get("shadow_risk_decisions", {})
                if isinstance(factors, dict)
                else {}
            )
            active = (
                factors.get("risk_decision", {}) if isinstance(factors, dict) else {}
            )
            account_equity = float(active.get("approved_risk_amount") or 0.0) / max(
                float(active.get("approved_risk_percent") or 0.0) / 100.0, 0.0001
            )
            counterfactual = DecisionEvidenceRepository().counterfactual_tier_outcomes(
                ...
                charges_per_unit=float(record.charges or 0.0)
                / max(int(record.filled_quantity or record.placed_quantity or 1), 1),
            )
    ...
    def _seconds_since_created(self, record: TradeRecord, value: datetime) -> float:
        ...
        if created_at.tzinfo is not None:
            created_at = created_at.astimezone(ZoneInfo("Asia/Kolkata")).replace(
                tzinfo=None
            )
    ...
    def exit_alerts(self, limit: int = 100) -> list[TradeRecord]:
        ...
        try:
            return (
                ...
                .filter(
                    TradeRecord.status.in_(
                        ["closing", "exit_failed", "reconciliation_mismatch"]
                    )
                )
    ...
    def mark_reconciliation_mismatch(
        self,
        trade_id: int,
        *,
        reason: str,
        ...
        try:
            ...
            record.exit_last_error = reason
            record.broker_status_json = json.dumps(
                broker_payload or {"reason": reason}, default=str
            )
            record.notes = reason
            record.updated_at = ist_now_naive()
