...
class EntryPolicy(Protocol):
    ...
    def evaluate(self, context: EntryPolicyContext, events: list[MarketPathEvent]) -> EntryPolicyDecision: ...
...
class CurrentBaselineEntryPolicy:
    ...
    def evaluate(self, context: EntryPolicyContext, events: list[MarketPathEvent]) -> EntryPolicyDecision:
        ...
        required_structure = "bullish" if direction in {"bullish", "buy_ce"} else "bearish"
        ...
        gates = {
            "completed_five_minute_directional": context.five_minute_structure.lower() == required_structure,
            "exact_one_five_agreement": context.one_minute_structure.lower() == required_structure,
            "premium_confirmation": context.premium_confirmation_passed,
            "preparation": context.preparation_passed,
            "fast_candidate_requirements": context.fast_candidate_requirements_passed if requires_fast_promotion else True,
            "promotion_registered": context.promotion_registered if requires_armed_confirmation else True,
            "armed_second_confirmation": context.armed_confirmation_passed if requires_armed_confirmation else True,
        ...
        state = "TRIGGERED" if enterable else "ARMED" if context.promotion_registered else "PREPARED" if context.preparation_passed else "OBSERVE"
...
class TransitionPreparationShadowPolicy:
    ...
    def evaluate(self, context: EntryPolicyContext, events: list[MarketPathEvent]) -> EntryPolicyDecision:
        ...
        five_allows_preparation = context.five_minute_structure.lower() in {expected, "neutral", "transition", "transitioning"}
        gates = {
            "five_minute_not_strongly_opposed": five_allows_preparation and not context.five_minute_strongly_opposed,
        ...
        return EntryPolicyDecision(
            ...
            features={"gates": gates, "event_count": len(events), "preparation_is_not_entry": True},
            timing_waterfall=_timing_waterfall(context, events),
        )
...
class ContinuousTransmissionShadowPolicy:
    ...
    def evaluate(self, context: EntryPolicyContext, events: list[MarketPathEvent]) -> EntryPolicyDecision:
        option_token = _optional_int(context.metadata.get("option_token") or context.metadata.get("instrument_token"))
        ...
        for event in sorted(events, key=lambda item: (item.exchange_timestamp, item.receive_timestamp)):
            if underlying_token is not None and event.instrument_token == underlying_token:
                if event.ltp is not None:
                    underlying_start = underlying_start if underlying_start is not None else event.ltp
                    underlying_latest = event.ltp
                    crossed = event.ltp >= context.underlying_trigger if direction_up else event.ltp <= context.underlying_trigger
            ...
            if option_token is not None and event.instrument_token == option_token:
                if event.bid is not None:
                    ...
                    if prior_bid is None or event.bid >= prior_bid:
                        non_regressing_bid_started = non_regressing_bid_started or event.exchange_timestamp
                    else:
                ...
                if underlying_cross_at and option_response_at is None and first_bid and event.bid and event.bid > first_bid:
        ...
        underlying_move_pct = (
            (underlying_latest - underlying_start) / max(abs(underlying_start), 0.01) * 100.0
        ...
        transmission_consistent = bool(bid_progressions >= 1 and latest_bid is not None and first_bid is not None and latest_bid >= first_bid)
        gates = {
            "prepared_context": context.price_plan_valid and context.base_risk_feasible and context.data_fresh and context.provenance_valid,
            ...
            "spread_stable": spread_pct is not None and spread_pct <= context.maximum_spread_pct,
            "bid_depth_safe": latest_bid_depth is not None and latest_bid_depth >= context.minimum_depth,
            "ask_depth_safe": latest_ask_depth is not None and latest_ask_depth >= context.minimum_depth,
            "chase": context.chase_valid,
            "target_room": context.target_room_valid,
            "remaining_rr": context.remaining_rr_valid,
            "session": context.session_eligible,
        ...
        failed = tuple(name for name, passed in gates.items() if not passed)
        enterable = not failed
        state = "TRIGGERED" if enterable else "ARMED" if underlying_cross_at else "PREPARED"
        features = {
            "gates": gates,
            "underlying_move_percent": underlying_move_pct,
            "option_bid_move_percent": bid_move_pct,
            "option_ask_move_percent": ask_move_pct,
            "mid_price_move_diagnostic_only": _mid_move(first_bid, first_ask, latest_bid, latest_ask),
            "bid_progression_count": bid_progressions,
            "ask_progression_count": ask_progressions,
            "non_regressing_bid_duration_ms": (
                (events[-1].exchange_timestamp - non_regressing_bid_started).total_seconds() * 1000.0
            ...
            "premium_move_per_underlying_move": (
                bid_move_pct / underlying_move_pct if bid_move_pct is not None and underlying_move_pct not in {None, 0.0} else None
            ),
            "response_consistency": sum(1 for left, right in zip(recent_bids, recent_bids[1:]) if right >= left) / max(len(recent_bids) - 1, 1),
            "recent_executable_bid_breakout": bool(recent_bids and latest_bid == max(recent_bids)),
        ...
        waterfall["policy_became_enterable"] = _iso(option_response_at) if enterable else None
        return EntryPolicyDecision(
            policy_version=self.version,
            policy_kind=self.kind,
            state=state,
...
class ShadowEntryPolicyComparisonService:
    ...
    def compare(self, context: EntryPolicyContext, events: list[MarketPathEvent], *, persist: bool = True) -> dict[str, Any]:
        ...
        return {
            ...
            "decisions": {decision.policy_version: decision.to_dict() for decision in decisions},
        }

    def _persist(self, context: EntryPolicyContext, decision: EntryPolicyDecision) -> None:
        ...
        try:
            record = (
                ...
                .filter(
                    ...
                    ShadowPolicyDecisionRecord.policy_version == decision.policy_version,
                )
                .first()
            )
            values = {
                ...
                "decision_json": json.dumps(decision.to_dict(), default=str, sort_keys=True),
                "timing_waterfall_json": json.dumps(decision.timing_waterfall, default=str, sort_keys=True),
...
def _timing_waterfall(context: EntryPolicyContext, events: list[MarketPathEvent]) -> dict[str, str | None]:
    metadata = context.metadata
    option_token = _optional_int(metadata.get("option_token") or metadata.get("instrument_token"))
    first_ask = next((event.exchange_timestamp for event in events if event.instrument_token == option_token and event.ask is not None), None)
    return {
        ...
        "underlying_trigger_crossed": _string_time(metadata.get("underlying_trigger_crossed_at")),
        "option_first_responded": _string_time(metadata.get("option_first_responded_at")),
        "premium_confirmation_passed": _string_time(metadata.get("premium_confirmation_at")),
        ...
        "armed_confirmation_passed": _string_time(metadata.get("armed_confirmation_at")),
...
def _mid_move(first_bid: float | None, first_ask: float | None, bid: float | None, ask: float | None) -> float | None:
