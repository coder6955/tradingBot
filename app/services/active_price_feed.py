...
class KitePollingPriceFeed:
    ...
    def __init__(self, provider: KiteProvider, market_data_coordinator: MarketDataCoordinator | None = None) -> None:
        ...
        *,
        ...
        try:
            if self.market_data_coordinator is not None:
                quote = self.market_data_coordinator.quote([instrument], provider=self.provider)
            else:
...
class ActiveTradePriceFeed:
    ...
    def __init__(self, websocket_feed: KiteWebSocketPriceFeed | None = None, market_data_coordinator: MarketDataCoordinator | None = None) -> None:
    ...
    def subscribe(self, tokens: set[int] | list[int] | tuple[int, ...]) -> dict[str, Any]:
    ...
    def unsubscribe(self, tokens: set[int] | list[int] | tuple[int, ...]) -> dict[str, Any]:
        clean_tokens = {int(token) for token in tokens if self._safe_token(token)}
    ...
    def latest_price(
        ...
        *,
        ...
        self.fallback_active = True
        self.fallback_count += 1
        tick = KitePollingPriceFeed(provider, self.market_data_coordinator).latest_price(exchange=exchange, tradingsymbol=tradingsymbol, instrument_token=instrument_token, mode=mode)
        self.last_tick = tick
    ...
    def status(self) -> dict[str, Any]:
        ...
        status = self.websocket_feed.status(active_trade_tokens=self.active_trade_tokens, fallback_active=self.fallback_active)
        status["last_reason"] = self.last_reason
        status["fallback_count"] = self.fallback_count
        return status

    def _websocket_tick(self, instrument_token: int | None, *, mode: str = "paper") -> WebSocketTick | None:
        if self.websocket_feed is None:
        ...
        if tick is None:
            self.last_reason = "token_missing"
            return None
        if not self.websocket_feed.is_fresh(int(instrument_token), settings.websocket_price_stale_seconds):
    ...
    def _to_price_tick(self, *, exchange: str, tradingsymbol: str, tick: WebSocketTick) -> PriceTick:
        return PriceTick(
            instrument=f"{exchange or settings.option_exchange}:{tradingsymbol}",
            price=tick.price,
