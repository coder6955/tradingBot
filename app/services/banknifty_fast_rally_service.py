...
class BankNiftyFastRallyService:
    ...
    def on_tick(self, tick: WebSocketTick) -> dict[str, Any] | None:
        if (
            self.underlying_token is None
            or int(tick.instrument_token) != self.underlying_token
        ):
            return None
        ...
        with self._lock:
            ...
            window = [
                (timestamp, value)
                for timestamp, value in self._ticks
            ...
            self.last_observation = {
                ...
                "threshold_progress_pct": round(
                    min(100.0, abs(move_pct) / threshold * 100.0), 2
                ),
                "direction": direction,
            ...
            self.trigger_count_by_direction[direction] = (
                self.trigger_count_by_direction.get(direction, 0) + 1
            )
            ...
            try:
                ...
                with self._lock:
                    ...
                    if bool(normalized_result.get("scheduled")):
                        ...
                        reason = str(
                            normalized_result.get("reason") or "callback_not_scheduled"
                        )
                        self.suppressed_reasons[reason] = (
                            self.suppressed_reasons.get(reason, 0) + 1
                        )
                ...
                with self._lock:
                    ...
                    event["dispatch"] = {
                        "scheduled": False,
                        "reason": "fast_rally_callback_failed",
                        **error,
                    }
                    self.last_event = dict(event)
    ...
    def status(self) -> dict[str, Any]:
        with self._lock:
            return {
                ...
                "last_callback_result": dict(self.last_callback_result)
                if self.last_callback_result
                else None,
                "last_callback_error": dict(self.last_callback_error)
                if self.last_callback_error
                else None,
