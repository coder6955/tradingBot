...
class DataFreshnessService:
    ...
    def validate_scan_inputs(
        ...
        *,
        ...
        if live and (not is_real or source in {"mock", "fallback"}):
            reasons.append(
                f"live mode requires fresh Kite quote; source={source or 'unknown'}, is_real_data={is_real}"
            )
        if live and not bool(snapshot.get("analysis_ready")):
            quality_reasons = snapshot.get("data_quality_reasons") or [
                "analysis snapshot unavailable"
            ]
            reasons.append(
                f"live mode requires canonical completed-candle analysis: {quality_reasons}"
            )

        quote_source = str(snapshot.get("quote_timestamp_source") or "unavailable")
        checks["banknifty_quote_timestamp_source"] = quote_source
        if live and quote_source not in {
            "exchange_timestamp",
            "last_trade_time",
            "timestamp",
        }:
            reasons.append(
                f"Bank Nifty quote lacks broker/exchange timestamp provenance: {quote_source}"
            )

        quote_age = self._age_seconds(
            snapshot.get("quote_timestamp") or snapshot.get("timestamp")
        )
        ...
        if live and quote_age is None:
            ...
            and quote_age > settings.max_live_quote_age_seconds
        ):
        ...
        max_candle_age = (
            ...
            if live
            else settings.max_paper_candle_age_seconds
        )
        ...
        if live and not chain_quotes:
            ...
            and chain_age > settings.max_live_chain_age_seconds
        ):
        ...
        if live and contract is not None and option_age is None:
            ...
            and option_age > settings.max_live_option_quote_age_seconds
        ):
    ...
    def _latest_candle_age(self, snapshot: dict[str, Any]) -> float | None:
        ...
        return self._age_seconds(
            snapshot.get("latest_candle_at") or snapshot.get("candle_timestamp")
        )

    def _selected_option_age(
        self, chain_quotes: dict[str, Any], contract: Any | None
    ) -> float | None:
        if contract is None:
            return None
        key = f"{getattr(contract, 'exchange', settings.option_exchange)}:{getattr(contract, 'tradingsymbol', '')}"
        payload = (
            chain_quotes.get(key)
            or chain_quotes.get(getattr(contract, "tradingsymbol", ""))
            or {}
        )
    ...
    def _quote_map_age(self, chain_quotes: dict[str, Any]) -> float | None:
        ages = [
            self._quote_age(value)
            for value in chain_quotes.values()
