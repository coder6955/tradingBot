...
class OpportunityAnalyticsService:
    ...
    def analyze(
        ...
        closed = [
            record for record in records if record.status == "closed" and record.outcome
        ]
        ...
        return {
            ...
            "lineage_warning": "Do not use the combined overall result for readiness; compare matching config hashes."
            ...
            "segments": {
                ...
                "time_bucket": self._groups(
                    closed, lambda record: time_bucket(record.created_at)
                ),
                "score_bucket": self._groups(
                    closed, lambda record: score_bucket(record.score)
                ),
    ...
    def _summary(self, records: list[OpportunityRecord]) -> dict[str, Any]:
        return summarize_returns(
            ...
            time_in_trade_fn=lambda record: minutes_between(
    ...
    def _data_quality(self, records: list[OpportunityRecord]) -> dict[str, Any]:
        ...
        return {
            ...
            "factor_score_coverage_pct": round(
                (len(with_factor_scores) / len(records)) * 100, 2
            )
            if records
            else 0.0,
    ...
    def _expiry_day(self, record: OpportunityRecord) -> str:
        ...
        return (
            "expiry_day"
            if record.created_at.date().isoformat() == str(record.expiry)[:10]
            else "non_expiry"
        )
