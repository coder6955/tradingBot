...
class EntryOpportunityService:
    ...
    def evaluate(
        ...
        *,
        ...
        observed_range = max(baseline_values) - min(baseline_values) if len(baseline_values) >= 2 else 0.0
        spread_scale = trigger * max(spread_pct, 0.05) / 100.0
        scale = max(float(volatility_scale or 0.0), observed_range, spread_scale * 3.0, trigger * 0.002, 0.15)
        ...
        if normalized_chase > settings.normalized_entry_chase_max_atr:
            blockers.extend(["entry_too_late", "chase_risk_high", "normalized_chase_risk_high"])
        ...
        if expected_move_coverage is not None and expected_move_coverage < settings.min_entry_expected_move_coverage:
            (warnings if breakout_accepted else blockers).append("expected_move_coverage_weak")
        if room_to_level_pct is not None and room_to_level_pct < settings.min_entry_room_to_level_pct:
            (warnings if breakout_accepted else blockers).append("nearest_level_room_too_small")
