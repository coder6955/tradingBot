...
class BankNiftyRegimeFilterService:
    ...
    def evaluate(
        ...
        *,
        ...
        volatility_details = (
            volatility_eval.get("details", {})
            if isinstance(volatility_eval, dict)
            and isinstance(volatility_eval.get("details"), dict)
            else {}
        )
        ...
        time_edge = self._time_of_day_context(
            time_bucket_eval, premium_eval, day_type_eval
        )
        decay = self._decay_context(
            prices, premium_eval, volatility_eval, volatility_details
        )
        ...
        hard_reasons = list(
            dict.fromkeys(str(reason) for reason in hard_reasons if str(reason))
        )
        soft_reasons = list(
            dict.fromkeys(str(reason) for reason in soft_reasons if str(reason))
        )
    ...
    def _opening_structure(
        self, bank_details: dict[str, Any], bullish: bool
    ) -> dict[str, Any]:
        opening = (
            bank_details.get("openingRangeStatus", {})
            if isinstance(bank_details.get("openingRangeStatus"), dict)
            else {}
        )
        ...
        soft: list[str] = []
        penalty = 0
        if status in {"failed_breakout", "failed_breakdown"} or (
            large_wick and status in {"opposite_side", "inside_opening_range"}
        ):
    ...
    def _gap_context(self, snapshot: dict[str, Any], bullish: bool) -> dict[str, Any]:
        ...
        if abs(gap_pct) >= settings.banknifty_significant_gap_pct and price > 0:
            ...
            failed_gap = (gap_pct > 0 and price < day_open and bullish) or (
                gap_pct < 0 and price > day_open and not bullish
            )
    ...
    def _vwap_context(
        self, snapshot: dict[str, Any], premium_details: dict[str, Any], bullish: bool
    ) -> dict[str, Any]:
        price = self._float(snapshot.get("price"))
        vwap = self._float(snapshot.get("vwap"))
        premium = self._float(
            premium_details.get("last_close") or premium_details.get("last_price")
        )
        ...
        if price > 0 and vwap > 0:
            ...
            if not supports:
                hard.append(
                    "banknifty_vwap_reclaim_not_confirmed"
                    if bullish
                    else "banknifty_vwap_rejection_not_confirmed"
                )
                penalty += 30
        if premium > 0 and premium_vwap > 0 and premium < premium_vwap:
            ...
            penalty += 25
        return {
            "banknifty_price": price,
            "banknifty_vwap": vwap,
            "option_premium": premium,
            "option_vwap": premium_vwap,
            "hard_reasons": hard,
            "soft_reasons": [],
            "penalty": penalty,
        }
    ...
    def _compression_expansion(
        self, day_type_eval: dict[str, Any], premium_details: dict[str, Any]
    ) -> dict[str, Any]:
        details = (
        ...
        soft: list[str] = []
        penalty = 0
        compressed = day_type in {"rotation_range", "range"} or (
            0 < day_range_pct < settings.banknifty_compression_day_range_pct
        )
        ...
        return {
            ...
            "premium_volume_expansion": volume_expansion,
            "hard_reasons": hard,
            "soft_reasons": soft,
            "penalty": penalty,
        }
    ...
    def _expiry_context(
        ...
        dte = (
            bank_details.get("dteMode", {})
            if isinstance(bank_details.get("dteMode"), dict)
            else {}
        )
        risk = str(dte.get("risk") or "unknown")
        premium_score = int(premium_eval.get("score") or 0)
        vol_score = (
            int(volatility_eval.get("score") or 100)
            if isinstance(volatility_eval, dict)
            else 100
        )
        hard: list[str] = []
        ...
        if (
            risk == "near_expiry"
            and premium_score < settings.banknifty_expiry_min_premium_score
        ):
            ...
            penalty += 8
        return {
            "days_to_expiry": dte.get("daysToExpiry"),
            "risk": risk,
    ...
    def _time_of_day_context(
        ...
        penalty = 0
        if now >= cutoff and (
            premium_score < settings.banknifty_late_trade_min_premium_score
            or day_type not in {"trend_expansion", "directional_acceptance"}
        ):
            hard.append("late_day_premium_decay_environment")
            penalty += 25
        if settings.enable_time_bucket_filter and not time_bucket_eval.get(
            "passed", False
        ):
            hard.extend(
                str(reason)
                for reason in time_bucket_eval.get(
                    "reasons", ["time bucket edge failed"]
                )
            )
            penalty += 20
        ...
        return {
            ...
            "late_cutoff": settings.banknifty_late_trade_cutoff_time,
            "hard_reasons": hard,
            "soft_reasons": soft,
            "penalty": penalty,
        }
    ...
    def _decay_context(
        ...
        premium_score = int(premium_eval.get("score") or 0)
        main_risk = (
            str(volatility_eval.get("main_risk") or "")
            if isinstance(volatility_eval, dict)
            else ""
        )
        best_coverage = self._float(
            volatility_details.get("best_expected_move_coverage")
        )
        hard: list[str] = []
        soft: list[str] = []
        penalty = 0
        ...
        if (
            best_coverage
            and best_coverage < settings.vol_edge_min_expected_move_coverage
            and premium_score < settings.banknifty_expiry_min_premium_score
        ):
        ...
        return {
            ...
            "premium_score": premium_score,
            "volatility_main_risk": main_risk,
            "best_expected_move_coverage": best_coverage,
            "hard_reasons": hard,
            "soft_reasons": soft,
            "penalty": penalty,
        }
    ...
    def _verdict(
        self, classification: str, hard_reasons: list[str], soft_reasons: list[str]
    ) -> str:
        if hard_reasons:
            return "Do not buy Bank Nifty options in this regime: " + "; ".join(
                hard_reasons[:3]
            )
        if soft_reasons:
            return (
                f"{classification}: valid only with strict entry timing; cautions: "
                + "; ".join(soft_reasons[:3])
            )
