...
class SetupFamilyClassifierService:
    ...
    def classify(
        ...
        *,
        ...
        if symbol.upper() != "BANKNIFTY" or side.upper() != "BUY":
            return self._result(
                "non_banknifty_or_non_buy_setup",
                "other",
                ["outside Bank Nifty option-buying taxonomy"],
            )
        ...
        evidence = {
            ...
            if contract
            else self._nested(factor_scores, "contract", "tradingsymbol"),
            "market_state": self._nested(factor_scores, "market_regime", "regime"),
            "market_state_confidence": self._nested(
                factor_scores, "market_regime", "confidence"
            ),
            "momentum_phase": self._nested(factor_scores, "momentum_phase", "phase"),
            "mtf_alignment_score": self._nested(
                factor_scores, "multi_timeframe", "alignment_score"
            ),
        }

        if (
            dte.get("risk") == "near_expiry"
            and premium_score >= 75
            and (premium_breakout or premium_volume)
        ):
            return self._result(
                "expiry_scalp_setup",
                "expiry",
                ["near expiry", "strong premium participation"],
                evidence,
            )

        opening_status = str(opening.get("status") or "")
        if (bullish and opening_status == "breakout") or (
            (not bullish) and opening_status == "breakdown"
        ):
            return self._result(
                "opening_breakout_continuation",
                "opening_drive",
                ["opening range continuation supports direction"],
                evidence,
            )
        ...
        if bullish and opening_status == "failed_breakdown":
            return self._result(
                ...
                "reversal",
                ["failed downside auction", "bullish reversal context"],
                evidence,
            )
        if (not bullish) and opening_status == "failed_breakout":
            return self._result(
                ...
                "reversal",
                ["failed upside auction", "bearish reversal context"],
                evidence,
            )

        compressed = str(compression.get("day_type") or day_type) in {
            "rotation_range",
            "range",
        }
        if compressed and premium_breakout and premium_volume:
            return self._result(
                ...
                ["range/compression expanding with premium participation"],
                evidence,
            )
        ...
        if vwap["banknifty_supports"] and vwap["premium_supports"]:
            if bullish:
                return self._result(
                    ...
                    "vwap_continuation",
                    ["Bank Nifty and option premium are above VWAP"],
                    evidence,
                )
            return self._result(
                "vwap_rejection_continuation",
                "vwap_continuation",
                ["Bank Nifty is below VWAP and option premium supports PE"],
                evidence,
            )
        ...
        if day_type in {"trend_expansion", "directional_acceptance"}:
            return self._result(
                ...
                "trend_day",
                ["directional/trend day context"],
                evidence,
            )

        label = (
            "normal_day_call_continuation" if bullish else "normal_day_put_continuation"
        )
        return self._result(
            label,
            "normal_day",
            ["valid directional setup without a stronger specialized family"],
            evidence,
        )
    ...
    def _policy(
        self, name: str, group: str, evidence: dict[str, Any]
    ) -> dict[str, Any]:
        ...
        unsuitable_state = market_state in {
            "balanced_rotation",
            "data_uncertain",
            "execution_untradable",
        }
        ...
        eligible = (
            score >= settings.setup_policy_min_score
            and not unsuitable_state
            and not exhausted
        )
        abstention = None
        ...
        profiles = {
            "opening_drive": {
                ...
                "trail_after_r": 1.0,
                "partial_at_r": 1.0,
                "target_style": "scale_on_expansion",
            },
            "compression_expansion": {
                "time_stop_minutes": 15,
                "trail_after_r": 1.2,
                "partial_at_r": 1.0,
                "target_style": "measured_move",
            },
            "vwap_continuation": {
                "time_stop_minutes": 15,
                "trail_after_r": 1.0,
                "partial_at_r": 1.0,
                "target_style": "vwap_structure",
            },
            "trend_day": {
                "time_stop_minutes": 22,
                "trail_after_r": 1.5,
                "partial_at_r": 1.0,
                "target_style": "runner",
            },
            "reversal": {
                ...
                "trail_after_r": 0.8,
                "partial_at_r": 0.8,
                "target_style": "fast_mean_reversion",
            },
            "expiry": {
                "time_stop_minutes": 7,
                "trail_after_r": 0.7,
    ...
    def _vwap_context(
        ...
        bank_supports = (
            price > 0
            and vwap > 0
            and ((bullish and price >= vwap) or ((not bullish) and price <= vwap))
        )
        premium_supports = premium > 0 and premium_vwap > 0 and premium >= premium_vwap
        return {
            "banknifty_supports": bank_supports,
            "premium_supports": premium_supports,
        }

    def _day_type(
        self, factor_scores: dict[str, Any], bank_details: dict[str, Any]
    ) -> str:
