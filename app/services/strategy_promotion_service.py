...
class StrategyPromotionService:
    ...
    def __init__(
        self,
        evidence: EvidenceMatrixService | None = None,
        validations: StrategyValidationRepository | None = None,
    ) -> None:
    ...
    def evaluate(self, *, timeframe: str = "5minute") -> dict[str, Any]:
        matrix = self.evidence.report(
            group_by=["setup_family", "market_regime", "direction"]
        )
        records = [
            row
            for row in self.validations.recent(limit=100)
            if row.strategy_version == settings.strategy_version
        ]
        current = [
            row
            for row in records
            ...
            and row.mode == "walk_forward_option"
        ]
        ...
        for direction in ("CALL", "PUT"):
            ...
            threshold_passed = bool(
                ...
                and float(row.expectancy_pct)
                >= settings.evidence_matrix_min_expectancy_pct
                and row.profit_factor is not None
                and float(row.profit_factor)
                >= settings.evidence_matrix_min_profit_factor
                and float(row.max_drawdown_pct)
                <= settings.evidence_matrix_max_drawdown_pct
            )
            checks.append(
                {
                    "direction": direction,
        ...
        sufficiently_segmented = any(
            bool(row.get("evidence_sufficient")) for row in matrix["groups"]
        )
        ...
        return {
            ...
            "evidence_matrix_summary": {
                ...
                "sufficient_groups": len(
                    [row for row in matrix["groups"] if row.get("evidence_sufficient")]
                ),
            },
            "reasons": reasons,
