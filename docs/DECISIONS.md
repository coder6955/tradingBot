...
## D055 — Notify scheduled startup only after end-to-end health verification

**Status:** Accepted by explicit operator direction  
**Decision:** The scheduled runner polls the bound local API for database health, verified Kite profile access, a connected WebSocket, the Bank Nifty `core_market` subscription, feed-gap safety, and a healthy automation-supervisor task. It sends one Telegram success notification only after every check passes, or a bounded failure notification after timeout, import failure, early server exit, or unexpected later shutdown.  
**Why:** A process-start or supervisor-start message is a false positive when login, broker data, the WebSocket, the database, or the trading worker is unavailable. The operator needs a remote, actionable distinction between "process launched" and "trading runtime ready."  
**Consequence:** Scheduled startup health waits up to 180 seconds by default and is configurable through `SCHEDULED_STARTUP_HEALTH_TIMEOUT_SECONDS` and `SCHEDULED_STARTUP_HEALTH_POLL_SECONDS`. Scheduled runs suppress the supervisor's immediate generic start alert to avoid contradictory messages. Telegram remains best-effort: if its credentials or network are unavailable, the notification result is written to the scheduled logs, which remain the recovery source of truth.

## D056 — Separate supervisor liveness from trading activity and enforce one runtime

**Status:** Accepted by explicit operator direction  
**Decision:** `automation.running` remains the internal supervisor-loop flag, while the public operator state separately reports `supervisor_alive`, `trading_active`, `day_complete`, execution profile, and completion policy. The dashboard consumes that derived state and is reduced to actionable runtime, risk, trade, setup, feed, research, notification, and decision views. An OS-level lock is acquired before any startup side effect, so a second Uvicorn process cannot create another trading runtime on the same or a different port.  
**Why:** A continuous supervisor waiting overnight was displayed as active automation after durable research had completed, and Windows can run FastAPI startup hooks before a duplicate Uvicorn bind fails. Raw liveness therefore confused the operator and a second manual start could briefly duplicate automation work.  
**Consequence:** `DAY_COMPLETE` is green only because trading and research work are finished; it never implies a live scanner. A manual start while the background process owns the lock fails before broker/WebSocket/automation startup. The operator must stop the existing runtime deliberately rather than changing ports.

## D057 — Alert on runtime incidents and prove shutdown before declaring completion
