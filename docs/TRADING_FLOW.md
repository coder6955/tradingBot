...
- The scheduled runner performs session-aware runtime checks after the qualified startup alert. API/database/supervisor failure is always actionable; Kite, WebSocket, `core_market`, and feed-gap checks become mandatory while live modules are expected. New worker exceptions open a Telegram incident, repeated identical incidents are suppressed in the same run, and recovery closes an active health incident.
- An OS-level instance lock is acquired before startup side effects. A second manual or scheduled Uvicorn launch cannot start another scanner/order runtime, even on another port. The existing process must be shut down deliberately before development startup.
...
For a Windows scheduled run, market-close outcome evaluation and the configured after-market research lane must report completion before the process exits. The scheduled runner then requests graceful API shutdown, verifies that shutdown hooks and persistence queues completed, and only then sends the Telegram safe-to-power-off notification. A failed shutdown sends a failure alert instead. Continuous/manual server runs retain the existing overnight supervisor behavior, but their dashboard operator state becomes `DAY_COMPLETE` with trading inactive rather than the misleading label “automation running.” A scheduler execution limit is only a hung-process fail-safe and is not the normal completion signal.
